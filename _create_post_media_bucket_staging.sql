-- CREATE POST-MEDIA STORAGE BUCKET FOR STAGING
-- Run this in Supabase Dashboard > SQL Editor for project oadwluspjlsnxhgakral
-- URL: https://supabase.com/dashboard/project/oadwluspjlsnxhgakral/sql/new

-- 1. Create the bucket
INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
VALUES (
  'post-media',
  'post-media',
  true,                -- Must be public for getPublicUrl() to work
  52428800,            -- 50 MB max per file
  ARRAY['image/*']
)
ON CONFLICT (id) DO UPDATE
  SET public = true,
      file_size_limit = EXCLUDED.file_size_limit,
      allowed_mime_types = EXCLUDED.allowed_mime_types;

-- 2. Policy: Authenticated users can upload
DROP POLICY IF EXISTS "post-media: authenticated insert" ON storage.objects;
CREATE POLICY "post-media: authenticated insert"
  ON storage.objects FOR INSERT
  TO authenticated
  WITH CHECK (bucket_id = 'post-media');

-- 3. Policy: Anyone can read (needed for public URLs)
DROP POLICY IF EXISTS "post-media: public select" ON storage.objects;
CREATE POLICY "post-media: public select"
  ON storage.objects FOR SELECT
  TO public
  USING (bucket_id = 'post-media');

-- 4. Policy: Owners can delete their files
DROP POLICY IF EXISTS "post-media: owner delete" ON storage.objects;
CREATE POLICY "post-media: owner delete"
  ON storage.objects FOR DELETE
  TO authenticated
  USING (bucket_id = 'post-media' AND owner = auth.uid());

-- After running this, check the bucket exists:
-- https://supabase.com/dashboard/project/oadwluspjlsnxhgakral/storage/buckets
