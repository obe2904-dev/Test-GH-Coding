...
async function checkTiming() {
  ...
  if (structured?.menuPeriods) {
    ...
    if (result.structured_data?.menuPeriods) {
      console.log(`\n🕐 Menu Periods (${result.structured_data.menuPeriods.length}):`)
      result.structured_data.menuPeriods.forEach((period, i) => {
        console.log(`  ${i + 1}. ${period.name} (${period.type})`)
        console.log(`     ⏰ ${period.startTime}-${period.endTime}`)
        console.log(`     📦 ${period.items?.length || 0} items`)
      })
    } else {
      console.log('❌ No menuPeriods found in structured_data')
    }
    
    if (result.structured_data?.availabilityTime) {
      console.log(`\n📋 Availability Time: ${result.structured_data.availabilityTime}`)
    }
  })
}
