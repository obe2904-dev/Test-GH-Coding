  ...
  try {
    ...
    const backgroundGeneration = (async ()=>{
      try {
        ...
        // Strategic distribution is now generated by AI in Phase 2c narrative
        // No need to rebuild mechanical sentence - AI explains the reasoning
        ...
        const { error: saveError } = await dataClient.from('weekly_strategies').upsert({
          ...
          strategy_rationale: (() => {
            const parts = []
            // Use AI-generated strategic_distribution instead of mechanical sentence
            if (strategy.narrative?.strategic_distribution) {
              parts.push(strategy.narrative.strategic_distribution)
            } else if (strategy.strategic_brief?.week_summary) {
              // Fallback to week_summary if strategic_distribution not available
              parts.push(strategy.strategic_brief.week_summary)
            }
