import type { WeekContext, BusinessArchetype, BusinessMode, LocationBehaviorMode, GuestOccasion, WeekModifiers, StrategicPriorityCandidate } from '../_shared/post-helpers/types/strategy-types.ts'
 ...
 */
export function calculateEconomicTiming(weekStartDate: Date): {
  week_of_month: number;
  pattern: 'salary_week' | 'normal_spend' | 'budget_conscious' | 'december_high' | 'july_vacation';
  is_july: boolean;
  payday_this_week: boolean;
  payday_day_name?: string;
} {
  ...
  let pattern: 'salary_week' | 'normal_spend' | 'budget_conscious' | 'december_high' | 'july_vacation';
...
export function economicRelevanceForArchetype(archetype: BusinessArchetype): 'high' | 'medium' | 'low' {
  if (['wine_bar', 'late_night_bar', 'dinner_restaurant', 'full_service_restaurant', 'evening_bar'].includes(archetype)) {
    return 'high';   // Evening/premium — payday unlocks real spending uplift
  }
  if (['fast_casual', 'lunch_restaurant', 'brunch_cafe', 'all_day_cafe'].includes(archetype)) {
 ...
 */
export function deriveBusinessMode(ctx: WeekContext): BusinessMode {
 ...
 */
export function deriveWeatherRelevance(ctx: WeekContext, businessMode: BusinessMode): {
  ...
  const avgRain = days.length > 0
    ? days.reduce((s, d) => s + (d.precipitation_chance ?? 0), 0) / days.length
    : 0;
  const avgWind = days.length > 0
    ? days.reduce((s, d) => s + (d.wind_speed ?? 0), 0) / days.length
    : 0;

  // visit_behavior
  let weather_effect_on_visit_behavior: 'indoor_refuge' | 'terrace_pull' | 'takeaway_pull' | 'minimal';
  if (hasTerrace && avgTemp > 16 && avgRain < 40 && avgWind < 7) {
    weather_effect_on_visit_behavior = 'terrace_pull';
  } else if (hasTakeaway && ['coffee_bar_takeaway', 'morning_cafe'].includes(businessMode) && (avgTemp < 10 || avgRain > 50)) {
    weather_effect_on_visit_behavior = 'takeaway_pull';
  } else if (['dinner_restaurant', 'evening_bar'].includes(businessMode) && !hasTerrace) {
    // Advance-booking model — weather at decision time is irrelevant
    weather_effect_on_visit_behavior = 'minimal';
  } else if (avgTemp < 12 || avgRain > 50) {
  ...
  // relevance
  let weather_relevance_for_business: 'low' | 'medium' | 'high';
  if (hasTerrace || ['coffee_bar_takeaway', 'morning_cafe'].includes(businessMode)) {
  ...
  let weather_effect_on_daypart: 'morning' | 'lunch' | 'evening' | 'all_day' | 'minimal';
  if (weather_effect_on_visit_behavior === 'minimal') {
    ...
    if (['all_day_cafe', 'hybrid_day_to_evening', 'brunch_lunch_cafe'].includes(businessMode)) {
  ...
  return { weather_relevance_for_business, weather_effect_on_daypart, weather_effect_on_visit_behavior };
}
 ...
 */
export function deriveSeasonalSignals(ctx: WeekContext, ingredientsInSeason: string[]): {
  ...
  const menuText = [
    ...
    ...ctx.signature_items.map(i => [i.name, i.description ?? ''].join(' ')),
  ].join(' ').toLowerCase();

  const menu_supported_seasonal_signals = ingredientsInSeason.filter(ingredient =>
    menuText.includes(ingredient.toLowerCase())
  );

  return { seasonal_mood_signals, menu_supported_seasonal_signals };
}
 ...
 */
export function deriveVisitMode(ctx: WeekContext, businessMode: BusinessMode): {
  ...
  let visit_mode: 'destination' | 'convenience' | 'mixed';
  if (['dinner_restaurant', 'evening_bar'].includes(businessMode) || ctx.location.tourist_context) {
    visit_mode = 'destination';
  } else if (['morning_cafe', 'coffee_bar_takeaway'].includes(businessMode)) {
  ...
  const MOTIVATION_GROUP_MAP: Array<{ keys: string[]; motivation: 'social' | 'pause' | 'meal' | 'treat' | 'discovery' }> = [
    { keys: ['socialt_samvær', 'social', 'group', 'gathering'],                        motivation: 'social' },
    { keys: ['forkælelse', 'treat', 'special', 'forkæle'],                            motivation: 'treat' },
    { keys: ['turist_oplevelse', 'destinationsbesøg', 'discovery', 'tourist', 'explore'], motivation: 'discovery' },
    { keys: ['arbejdsfrokost', 'frokostnødvendighed', 'pause', 'work', 'break', 'ritual'], motivation: 'pause' },
    { keys: ['dinner', 'dining', 'meal'],                                               motivation: 'meal' },
  ];

  let primary_visit_motivation: 'social' | 'pause' | 'meal' | 'treat' | 'discovery' = 'meal';
  ...
  if (motivations.length === 0) {
    if (['morning_cafe', 'coffee_bar_takeaway'].includes(businessMode)) primary_visit_motivation = 'pause';
    else if (['dinner_restaurant', 'evening_bar'].includes(businessMode)) primary_visit_motivation = 'social';
    else primary_visit_motivation = 'meal';
  }

  // Secondary: motivations that map to a different group than primary
  const resolveGroup = (mot: string): string => {
    for (const group of MOTIVATION_GROUP_MAP) {
  ...
  return { visit_mode, primary_visit_motivation, secondary_visit_motivations };
}
 ...
 */
export function deriveDaypartPriority(
  ctx: WeekContext,
  businessMode: BusinessMode,
  visitMode: 'destination' | 'convenience' | 'mixed',
  weatherEffectOnDaypart: 'morning' | 'lunch' | 'evening' | 'all_day' | 'minimal'
): {
  ...
  if (Object.keys(scores).length === 0) {
    if (['morning_cafe', 'coffee_bar_takeaway'].includes(businessMode)) scores['morning'] = 1.0;
    else if (['dinner_restaurant', 'evening_bar'].includes(businessMode)) scores['evening'] = 1.0;
    else if (businessMode === 'lunch_restaurant') scores['lunch'] = 1.0;
    else { scores['lunch'] = 1.0; scores['evening'] = 1.0; }
  }
  ...
  if (ctx.economic.pattern === 'salary_week') {
    if (['dinner_restaurant', 'evening_bar'].includes(businessMode) && scores['evening'] !== undefined) {
      scores['evening'] += 0.3;
    }
    if (['morning_cafe', 'coffee_bar_takeaway'].includes(businessMode) && scores['morning'] !== undefined) {
      scores['morning'] += 0.2;
    }
  }

  // Event modifier
  const closestEvent = ctx.events.length > 0
    ? ctx.events.reduce((a, b) => (a.days_away <= b.days_away ? a : b))
    : null;
  ...
  if ((sp?.weeks_analyzed ?? 0) >= 3) {
    ...
    if ((rates['drive_footfall'] ?? 0) >= 0.5) {
      const trafficDaypart = ['dinner_restaurant', 'evening_bar'].includes(businessMode) ? 'evening' : 'lunch';
  ...
  const labelMap: Record<string, string> = {
    morning: 'morgen', lunch: 'frokost', afternoon: 'eftermiddag',
    early_evening: 'tidlig aften', evening: 'aften', late_night: 'sen aften',
  };
  const reasons: string[] = [];
  if (weatherEffectOnDaypart !== 'minimal') {
    const wEffect = (ctx as any).weather_effect_on_visit_behavior as string | undefined;
    const wNote: Record<string, string> = {
  ...
  const daypart_reasoning = reasons.length > 0
    ? `${reasons.join(' + ')} → ${primaryLabel} er kommercielt stærkest denne uge`
    : `${primaryLabel} er standardprioritering for dette forretningsformat`;

  return { primary_daypart_this_week, secondary_daypart_this_week, daypart_reasoning };
}
 ...
 */
export function buildStrategicPriorityCandidates(
  ctx: WeekContext,
  businessMode: BusinessMode,
  visitMode: 'destination' | 'convenience' | 'mixed',
  primaryDaypart: string,
  weatherRelevance: 'low' | 'medium' | 'high',
  weatherVisitBehavior: 'indoor_refuge' | 'terrace_pull' | 'takeaway_pull' | 'minimal',
  economicRelevance: 'high' | 'medium' | 'low',
  menuSupportedSeasonalSignals: string[]
  ...
  if (weatherVisitBehavior === 'indoor_refuge') {
    ...
    candidates.push({
      ...
      daypart_relevance: [primaryDaypart],
      menu_relevance: ['comforting_mains', 'warm_dishes', 'seasonal_menu'],
  ...
  if (weatherVisitBehavior === 'terrace_pull' && ctx.location.has_outdoor_seating) {
    candidates.push({
      ...
      daypart_relevance: [primaryDaypart, 'all_day'],
      menu_relevance: ['drinks', 'light_bites', 'seasonal_menu'],
      weather_relevance: 'high',
  ...
  if (weatherVisitBehavior === 'takeaway_pull') {
    candidates.push({
      ...
      daypart_relevance: ['morning', 'lunch'],
      menu_relevance: ['coffee', 'takeaway_items', 'morning_menu'],
      weather_relevance: 'high',
  ...
  if (ctx.economic.payday_this_week && economicRelevance === 'high') {
    ...
    candidates.push({
      ...
      daypart_relevance: ['evening'],
      menu_relevance: ['signature_dishes', 'wine', 'premium_mains'],
  ...
  if (ctx.economic.payday_this_week && economicRelevance === 'medium') {
    candidates.push({
      ...
      daypart_relevance: [primaryDaypart],
      menu_relevance: ['lunch_classics', 'daily_specials'],
      weather_relevance: 'low',
      location_relevance: 'low',
      confidence: 0.65,
    });
  ...
  if (visitMode === 'destination' && hasWeekend) {
    candidates.push({
      ...
      daypart_relevance: hasBrunch ? ['morning', 'lunch'] : ['evening'],
      menu_relevance: ['signature_dishes', 'brunch_menu', 'sharing_plates'],
  ...
  for (const event of holidayEventsInWindow) {
    ...
    candidates.push({
      ...
      daypart_relevance: [primaryDaypart],
      menu_relevance: ['seasonal_menu', 'signature_dishes'],
      weather_relevance: 'low',
      location_relevance: 'medium',
      confidence: 0.85,
    });
  ...
  if (['morning_cafe', 'coffee_bar_takeaway'].includes(businessMode) && primaryDaypart === 'morning') {
    candidates.push({
      ...
      daypart_relevance: ['morning'],
      menu_relevance: ['coffee', 'morning_menu', 'pastries'],
      weather_relevance: weatherRelevance,
      location_relevance: 'medium',
      confidence: 0.72,
    });
  ...
  if ((hasLunch || hasBrunch) && visitMode !== 'destination' && ctx.previous_week?.selection_patterns) {
    const pauseMotivations = ['arbejdsfrokost', 'frokostnødvendighed', 'pause'];
    const hasPauseMotivation = (ctx.location.matched_motivations ?? []).some(m =>
      pauseMotivations.some(p => m === p || m.includes(p))
    );
    if (hasPauseMotivation) {
      candidates.push({
        ...
        daypart_relevance: ['lunch'],
        menu_relevance: ['lunch_classics', 'daily_specials', 'quick_lunch'],
        weather_relevance: 'low',
  ...
  const buildBrandRate = ((sp?.goal_mode_rates as any)?.build_brand ?? 0) as number;
  ...
  if ((sp?.weeks_analyzed ?? 0) >= 2 && buildBrandRate < baselineBuildBrand - 0.15) {
    candidates.push({
      ...
      daypart_relevance: [primaryDaypart],
      menu_relevance: ['behind_scenes', 'team_story'],
      weather_relevance: 'low',
      location_relevance: 'medium',
      confidence: 0.58,
    });
  ...
  return candidates
    .map(c => {
      ...
      const daypartOverlap = c.daypart_relevance.some(d =>
        d === primaryDaypart || d === 'all_day'
      );
      if (!daypartOverlap) conf -= 0.15;
      return { ...c, confidence: Math.max(0, Math.min(1, conf)) };
    })
    .filter(c => c.confidence >= 0.40)
 ...
 */
export function deriveWeeklyFraming(
  locationBehaviorMode: LocationBehaviorMode,
  primaryVisitMotivation: 'social' | 'pause' | 'meal' | 'treat' | 'discovery',
  secondaryMotivations: string[],
  primaryDaypart: string,
  secondaryDaypart: string,
  businessMode: BusinessMode,
  economicRelevance: 'high' | 'medium' | 'low',
  ctx: WeekContext
): { location_framing: string; motivation_framing: string; daypart_framing: string } {
  // ─ Location framing: synthesise mode + enriching context ───────────────────
  const LOCATION_BASE: Record<LocationBehaviorMode, string> = {
  ...
  const MOTIVATION_BASE: Record<string, string> = {
  ...
  if (ctx.economic.payday_this_week && economicRelevance !== 'low') {
    const boost = economicRelevance === 'high'
      ? (primaryVisitMotivation === 'treat' ? 'premiumvalg og aftenbooking' : 'premium- og aftenappetitten')
      : 'sandsynlighed for at vælge lidt opad';
  ...
  const DAYPART_LABEL: Record<string, string> = {
    morning: 'morgen', lunch: 'frokost', afternoon: 'eftermiddag',
    early_evening: 'tidlig aften', evening: 'aften', late_night: 'sen aften', all_day: 'hele dagen',
  };
  ...
  const weekdayCount = availDays.filter(d => { const day = new Date(d).getDay(); return day >= 1 && day <= 5; }).length;
  const weekendCount = availDays.filter(d => { const day = new Date(d).getDay(); return day === 0 || day === 6; }).length;
  let daypart_framing: string;
  ...
  return { location_framing, motivation_framing, daypart_framing };
}
 ...
 */
export function deriveBusinessDriverRanking(
  businessMode: BusinessMode,
  locationBehaviorMode: LocationBehaviorMode,
  primaryVisitMotivation: 'social' | 'pause' | 'meal' | 'treat' | 'discovery',
  primaryDaypart: string,
  ...
  economicRelevance: 'high' | 'medium' | 'low',
  ctx: WeekContext
): { primary_driver: string; secondary_driver: string; supporting_drivers: string[]; deprioritized_drivers: string[] } {
  // Tier 1 — business identity: always the primary anchor
  const businessDesc = `${businessMode.replace(/_/g, '_')}` as string;
  const BUSINESS_MODE_LABEL: Record<string, string> = {
  ...
  const supporting_drivers: string[] = [];
  ...
  const deprioritized_drivers: string[] = [];
  ...
  return { primary_driver, secondary_driver, supporting_drivers, deprioritized_drivers };
}
 ...
 */
export function deriveLocationBehaviorMode(ctx: WeekContext): LocationBehaviorMode {
  const locType = ctx.location?.type;
  const primaryDaypart = (ctx as any).primary_daypart_this_week as string | undefined;
 ...
 */
export function deriveWeeklyInterpretation(ctx: WeekContext): {
  business_archetype: BusinessArchetype;
  business_mode: BusinessMode;
  core_business_drivers: Array<{ driver: string; always_relevant: boolean; amplified_by?: string[] }>;
  primary_guest_occasions: GuestOccasion[];
  week_modifiers: WeekModifiers;
  top_weekly_priorities: string[];
  ...
  business_driver_ranking: { primary_driver: string; secondary_driver: string; supporting_drivers: string[]; deprioritized_drivers: string[] };
  weekly_framing: { location_framing: string; motivation_framing: string; daypart_framing: string };
} {
  ...
  let business_archetype: BusinessArchetype;
  ...
  } = deriveWeatherRelevance(ctx, business_mode);

  // ── 1d. Seasonal Signals Split ────────────────────────────────────────────
  const { seasonal_mood_signals, menu_supported_seasonal_signals } =
    deriveSeasonalSignals(ctx, ctx.season.ingredients_in_season);

  // ── 2. Core Business Drivers ─────────────────────────────────────────────
  const core_business_drivers: Array<{ driver: string; always_relevant: boolean; amplified_by?: string[] }> = [];
  const strategy = ctx.brand_voice?.content_strategy;

  if (strategy?.brand_anchors) {
    for (const anchor of strategy.brand_anchors) {
      core_business_drivers.push({ driver: anchor, always_relevant: true });
    }
  }
  if (strategy?.footfall_signals) {
    for (const signal of strategy.footfall_signals) {
      core_business_drivers.push({
        ...
        amplified_by: ['weekend', 'payday', 'sunny'],
      });
    }
  }
  ...
  if (ctx.location.has_outdoor_seating) {
    core_business_drivers.push({
      ...
      always_relevant: false,
      amplified_by: ['sunny', 'warm', 'weekend'],
    });
  }
  // Always have at least one generic driver if none were set
  if (core_business_drivers.length === 0) {
    core_business_drivers.push({ driver: ctx.business_name, always_relevant: true });
  }

  // ── 3. Primary Guest Occasions ────────────────────────────────────────────
  const primary_guest_occasions: GuestOccasion[] = [];
  const motivations = ctx.location.matched_motivations ?? [];

  const MOTIVATION_OCCASION_MAP: Record<string, { occasion: string; primary: boolean; day_pattern: GuestOccasion['day_pattern'] }> = {
    socialt_samvær:     { occasion: 'Socialt samvær med venner/kolleger',  primary: true,  day_pattern: 'any' },
    frokostnødvendighed:{ occasion: 'Hverdagsfrokost',                     primary: true,  day_pattern: 'weekday' },
    destinationsbesøg:  { occasion: 'Destination og udflugt',              primary: true,  day_pattern: 'weekend' },
    familieudflugt:     { occasion: 'Familieudflugt',                      primary: false, day_pattern: 'weekend' },
    hygge_lokal:        { occasion: 'Lokal hyggestemning',                 primary: true,  day_pattern: 'any' },
    turist_oplevelse:   { occasion: 'Turistoplevelse',                     primary: false, day_pattern: 'any' },
    arbejdsfrokost:     { occasion: 'Arbejdsfrokost/-møde',                primary: false, day_pattern: 'weekday' },
    forkælelse:         { occasion: 'Forkælelse og særlige øjeblikke',     primary: false, day_pattern: 'weekend' },
    discovery:          { occasion: 'Opdagelse og nyt sted',               primary: false, day_pattern: 'any' },
  };

  for (const mot of motivations) {
    const mapped = MOTIVATION_OCCASION_MAP[mot];
    if (mapped) primary_guest_occasions.push({ ...mapped });
  }

  // Add service-period based occasions if none from motivations
  if (primary_guest_occasions.length === 0) {
    if (hasBrunch || hasBreakfast) {
      primary_guest_occasions.push({ occasion: 'Weekendbrunch', primary: true, day_pattern: 'weekend' });
    }
    if (hasLunch) {
      primary_guest_occasions.push({ occasion: 'Hverdagsfrokost', primary: true, day_pattern: 'weekday' });
    }
    if (hasDinner) {
      primary_guest_occasions.push({ occasion: 'Aftensmad ude', primary: true, day_pattern: 'any' });
    }
  }

  // ── 4. Week Modifiers ─────────────────────────────────────────────────────
  // Economic signal
  const economic_signal: WeekModifiers['economic_signal'] =
    ctx.economic.payday_this_week ? 'push' :
    ctx.economic.pattern === 'budget_conscious' ? 'none' : 'neutral';

  // Event weight — look at closest upcoming event
  const closestEvent = ctx.events.length > 0
    ? ctx.events.reduce((a, b) => (a.days_away <= b.days_away ? a : b))
    : null;
  const event_weight: WeekModifiers['event_weight'] =
    !closestEvent ? 'none' :
    closestEvent.days_away <= 3 ? 'high' :
    closestEvent.days_away <= 7 ? 'medium' : 'low';

  // Weather opportunity
  const wi = ctx.weather_interpretation;
  const weather_opportunity: WeekModifiers['weather_opportunity'] =
    !wi ? 'normal' :
    (wi.indoor_outdoor_bias === 'strongly_outdoor' || wi.indoor_outdoor_bias === 'lean_outdoor') ? 'strong' :
    (wi.indoor_outdoor_bias === 'strongly_indoor') ? 'constrained' : 'normal';

  // Overall priority
  const overallScore =
    (economic_signal === 'push' ? 2 : economic_signal === 'neutral' ? 1 : 0) +
    (event_weight === 'high' ? 2 : event_weight === 'medium' ? 1 : 0) +
    (weather_opportunity === 'strong' ? 1 : weather_opportunity === 'constrained' ? -1 : 0);

  const overall_priority: WeekModifiers['overall_priority'] =
    overallScore >= 3 ? 'high' : overallScore <= 0 ? 'low' : 'normal';

  const week_modifiers: WeekModifiers = { economic_signal, event_weight, weather_opportunity, overall_priority };
  ...
  const narrative_guardrails: string[] = [];
  ...
  const economic_relevance_for_business: 'high' | 'medium' | 'low' =
    ctx.economic.payday_this_week ? archetype_economic_relevance : 'low';

  // ── 9. Visit Mode ─────────────────────────────────────────────────────────
  const { visit_mode, primary_visit_motivation, secondary_visit_motivations } =
    deriveVisitMode(ctx, business_mode);

  // ── 10. Daypart Priority ─────────────────────────────────────────────────
  const { primary_daypart_this_week, secondary_daypart_this_week, daypart_reasoning } =
    deriveDaypartPriority(ctx, business_mode, visit_mode, weather_effect_on_daypart);

  // ── 11. Location Behavior Mode ────────────────────────────────────────────────
  // Needs primary_daypart_this_week already computed (step 10)
  const location_behavior_mode = deriveLocationBehaviorMode({ ...ctx, primary_daypart_this_week } as WeekContext);

  // ── 12. Structured Strategic Candidates ─────────────────────────────────────
  const strategic_priority_candidates_v2 = buildStrategicPriorityCandidates(
    ctx,
    business_mode,
    visit_mode,
  ...
  const business_driver_ranking = deriveBusinessDriverRanking(
    business_mode,
  ...
  const weekly_framing = deriveWeeklyFraming(
    location_behavior_mode,
    primary_visit_motivation,
...
export function getRealSeasonContext(weekStart: string, country: string): { current: 'winter' | 'spring' | 'summer' | 'autumn'; ingredients_in_season: string[]; out_of_season: string[] } {
  const month = new Date(weekStart).getMonth() + 1;
  
  const DK_SEASONS: Record<number, { current: 'winter' | 'spring' | 'summer' | 'autumn'; ingredients_in_season: string[]; out_of_season: string[] }> = {
    12: { current: 'winter', ingredients_in_season: ['kål', 'rodfrugter', 'æbler', 'pærer', 'vildand'], out_of_season: ['jordbær', 'tomater', 'asparges'] },
    1:  { current: 'winter', ingredients_in_season: ['kål', 'rodfrugter', 'selleri', 'porrer'], out_of_season: ['jordbær', 'tomater', 'agurk'] },
    2:  { current: 'winter', ingredients_in_season: ['kål', 'rodfrugter', 'pærer', 'citrusfrugter'], out_of_season: ['jordbær', 'hindbær', 'tomater'] },
    3:  { current: 'spring', ingredients_in_season: ['forårsløg', 'spinat', 'radiser', 'ramsløg'], out_of_season: ['rodfrugter', 'kål'] },
    4:  { current: 'spring', ingredients_in_season: ['asparges', 'ramsløg', 'spinat', 'ærter'], out_of_season: [] },
    5:  { current: 'spring', ingredients_in_season: ['asparges', 'jordbær', 'ærter', 'nye kartofler'], out_of_season: [] },
    6:  { current: 'summer', ingredients_in_season: ['jordbær', 'hindbær', 'tomater', 'nye kartofler', 'agurk'], out_of_season: ['kål', 'rodfrugter'] },
    7:  { current: 'summer', ingredients_in_season: ['hindbær', 'blåbær', 'tomater', 'zucchini', 'majs'], out_of_season: [] },
    8:  { current: 'summer', ingredients_in_season: ['blommetomater', 'majs', 'figner', 'svampe'], out_of_season: [] },
    9:  { current: 'autumn', ingredients_in_season: ['svampe', 'æbler', 'pærer', 'blommer', 'græskar'], out_of_season: ['tomater', 'agurk'] },
    10: { current: 'autumn', ingredients_in_season: ['græskar', 'svampe', 'æbler', 'vildand', 'kål'], out_of_season: [] },
    11: { current: 'autumn', ingredients_in_season: ['kål', 'rodfrugter', 'vildand', 'vildsvin', 'æbler'], out_of_season: ['jordbær', 'tomater'] },
  };
  
  return DK_SEASONS[month] || { current: 'spring', ingredients_in_season: [], out_of_season: [] };
}
