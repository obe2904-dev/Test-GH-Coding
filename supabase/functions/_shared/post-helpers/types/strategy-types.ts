...
/**
 * Content style dimension — how the content tells the story
 * Separates tactical CTA from narrative approach
 */
export type ContentStyle = 'performance_driven' | 'brand_building' | 'balanced';

/**
 * Goal mode — simplified to remove retain_loyalty (brand outcome, not tactical goal)
 */
...
export interface StrategicAngle {
  ...
  goal_mode?: GoalMode;  // Business goal for this post (drive_bookings | drive_footfall | build_brand)
  content_style?: ContentStyle; // NEW: How content tells the story (performance_driven | brand_building | balanced)
...
export interface PostIdea {
  ...
  goal_mode?: GoalMode;  // Simplified: drive_bookings | drive_footfall | build_brand
  content_style?: ContentStyle; // NEW: performance_driven | brand_building | balanced
