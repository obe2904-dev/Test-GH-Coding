...
`,
  ...
  const atmosphereHintMap: Record<string, string> = {
    da: 'STEMNINGSOPSLAG UDEN MENUINFORMATION: Opfind IKKE stedsatmosfære, location-fakta ELLER abstrakt stemning. Abstrakte stemningsord som "hygge", "varme", "ro", "hverdagen trænger til" er forbudt som åbner. 🚫 FORBUD MOD KØKKENSCENER: Beskriv IKKE køkkenarbejde, kokkens hænder, madlavning eller forberedelse (snitter, hakker, forbereder grøntsager) — disse hører til behind-the-scenes-opslag med verificeret køkkenindhold. Brug i stedet ét KONKRET element som ankerpunkt: en handling der sker i lokalet, en fysisk genstand, et tidspunkt eller et faktum om stedet. Reducer til: (1) konkret element fra INDHOLD eller BRANDSTEMME-blokken, (2) ét faktuelt tilbud fra ÅBNINGSTID hvis tilgængeligt, (3) CTA.',
    sv: 'STÄMNINGSINLÄGG UTAN MENYINFORMATION: Uppfinn INTE platsatmosphere, platsfakta ELLER abstrakt stämning. Abstrakta stämningsord som "mysighet", "värme", "ro", "vardagen behöver" är förbjudna som öppning. 🚫 FÖRBUD MOT KÖKSSCENER: Beskriv INTE köksarbete, kockens händer, matlagning eller förberedelse — dessa hör till behind-the-scenes-inlägg med verifierat köksinnehåll. Använd istället ett KONKRET element som ankpunkt: en handling som sker i lokalen, ett fysiskt föremål, en tidpunkt eller ett faktum om stället. Reducera till: (1) konkret element från INNEHÅLL eller VARUMÄRKESRÖST-blocket, (2) ett faktabaserat erbjudande från ÖPPETTIDER om tillgängligt, (3) CTA.',
    de: 'STIMMUNGSBEITRAG OHNE MENÜINFORMATION: Erfinde KEINE Ortsatmosphäre, Ortsfakten ODER abstrakte Stimmung. Abstrakte Stimmungswörter wie "Gemütlichkeit", "Wärme", "Ruhe", "der Alltag braucht" sind als Eröffnung verboten. 🚫 VERBOT VON KÜCHENSZENEN: Beschreibe NICHT Küchenarbeit, Kochs Hände, Kochen oder Vorbereitung — diese gehören zu Behind-the-Scenes-Beiträgen mit verifiziertem Kücheninhalt. Verwende stattdessen ein KONKRETES Element als Ankerpunkt: eine Handlung im Lokal, ein physischer Gegenstand, eine Uhrzeit oder eine Tatsache über den Ort. Reduziere auf: (1) konkretes Element aus INHALT oder MARKENSTIMME-Block, (2) ein faktisches Angebot aus ÖFFNUNGSZEITEN wenn verfügbar, (3) CTA.',
  }
