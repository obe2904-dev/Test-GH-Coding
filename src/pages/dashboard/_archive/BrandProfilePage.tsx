...
import { WritingGuidelinesPanel } from './businessProfile/components/WritingGuidelinesPanel'
...
export default function BrandProfilePage() {
  ...
  // const [targetAudience, setTargetAudience] = useState('') // unused
  // const [ctaPreference, setCtaPreference] = useState('') // cta_style field removed
  ...
  const [writingGuidelines, setWritingGuidelines] = useState<WritingGuidelines | null>(null)
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      try {
        ...
        // Fetch full brand profile for voice data
        const { data: fullProfile } = await supabase
          .from('business_brand_profile')
          .select('tone_model, voice_constraints, typical_openings, signature_phrases, humor_level, voice_rationale, content_strategy')
          .eq('business_id', businessId)
          .maybeSingle() as { data: any }
        
        if (fullProfile && isActive) {
          // Parse tone_model for writing rules and content anchors
          const toneModel = fullProfile.tone_model as any
          const contentStrategy = fullProfile.content_strategy as any
          const writingRules = Array.isArray(toneModel?.writing_rules) ? toneModel.writing_rules : []
          let contentAnchors = Array.isArray(toneModel?.content_anchors) ? toneModel.content_anchors : []
          // Content strategy anchors override tone_model anchors
          if (Array.isArray(contentStrategy?.anchors) && contentStrategy.anchors.length > 0) {
            contentAnchors = contentStrategy.anchors
          }
          
          // Parse voice_constraints (could be string or {value: string} object)
          const vc = fullProfile.voice_constraints as any
          const voiceConstraints = typeof vc === 'string' ? vc
            : (typeof vc === 'object' && typeof vc?.value === 'string') ? vc.value
            : ''
          
          // Parse voice_rationale (could be string or {value: string} object)
          const vr = fullProfile.voice_rationale as any
          const voiceRationale = typeof vr === 'string' ? vr
            : (typeof vr === 'object' && typeof vr?.value === 'string') ? vr.value
            : undefined
          
          setWritingGuidelines({
            writing_rules: writingRules,
            voice_constraints: voiceConstraints,
            typical_openings: Array.isArray(fullProfile.typical_openings) ? fullProfile.typical_openings : [],
            signature_phrases: Array.isArray(fullProfile.signature_phrases) ? fullProfile.signature_phrases : [],
            content_anchors: contentAnchors,
            humor_level: fullProfile.humor_level || 'moderate',
            voice_rationale: voiceRationale,
          })
        }
    ...
    try {
      ...
      const { error: brandError } = await (supabase
        ...
        .upsert({
          ...
          // cta_preference: ctaPreference.trim() || null, // field removed
          updated_at: new Date().toISOString()
    ...
    <div className="bg-gradient-to-br from-slate-50 to-slate-100 min-h-full py-6 px-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="space-y-4">
          ...
          {(socialStyle || voiceExamples || writingGuidelines) && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {writingGuidelines && <WritingGuidelinesPanel guidelines={writingGuidelines} />}
