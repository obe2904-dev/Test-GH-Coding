...
export function PlatformConnectionsCard({
  ...
  const borderColorClass = active ? 'border-[#0F2E32]' : 'border-gray-200'
        ...
        <div className="grid grid-cols-2 gap-2">
          {(['facebook', 'instagram'] as const).map((platform) => {
            ...
            const platformCopy = platform === 'facebook' ? {
              label: 'Facebook',
              icon: (
                <svg className={`w-4 h-4 ${connected ? 'text-[#0F2E32]' : 'text-[#9CA3AF]'}`} fill="currentColor" viewBox="0 0 24 24">
            ...
            return (
              <label
                ...
                className={`flex items-center justify-between px-3 py-2 border rounded-lg cursor-pointer transition-colors h-10 ${
                  connected ? 'border-[#C7BAF7] bg-[#F4F1FE]' : 'border-[#E5E7EB] bg-white hover:bg-[#F9FAFB]'
                }`}
              >
                <div className="flex items-center gap-2">
                  {platformCopy.icon}
                  <span className={`text-xs font-medium ${connected ? 'text-[#0F2E32]' : 'text-[#374151]'}`}>
                    {platformCopy.label}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  {isSaving && (
                    <div className="w-3 h-3 border-2 border-[#0F2E32] border-t-transparent rounded-full animate-spin" />
                  )}
                  {wasJustSaved && (
                  ...
                  <div className={`w-4 h-4 rounded flex items-center justify-center ${
                    connected ? 'bg-[#88F2D7]' : 'bg-white border border-[#D1D5DB]'
                  }`}>
                    {connected && (
                      <svg className="w-3 h-3 text-[#0F2E32]" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="3">
