...
if (data && data.post_ideas && data.post_ideas.length > 0) {
  console.log('📊 Post Ideas - Service Period Check:')
  data.post_ideas.forEach((idea, idx) => {
    console.log(`\n${idx + 1}. ${idea.title}`)
    console.log(`   service_period: ${idea.service_period || '❌ NOT SET'}`)
    console.log(`   goal_mode: ${idea.goal_mode}`)
    console.log(`   suggested_time: ${idea.suggested_time}`)
  })
} else {
